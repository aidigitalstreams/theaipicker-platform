export default function PageHero({ label, title, subtitle, dark = true }: { label?: string; title: string; subtitle?: string; dark?: boolean }) {
  const bg = dark ? '#0A0F1E' : '#F8FAFC';
  const txtColor = dark ? '#F8FAFC' : '#0F172A';
  ...
  return (
    <section style={{
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '800px', margin: '0 auto' }}>
        ...
        <h1 style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: '3rem', fontWeight: 800, color: txtColor, margin: '0 0 1rem', letterSpacing: '-1.5px', lineHeight: 1.1 }}>
          {title}
        </h1>
        {subtitle && (
          <p style={{ fontSize: '1.125rem', color: subColor, lineHeight: 1.6, maxWidth: '600px', margin: '0 auto' }}>{subtitle}</p>
        )}
      </div>
    </section>
  );
