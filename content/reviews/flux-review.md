---
title: 'Flux Review (2026): Black Forest Labs'' Prompt-Adherence Champion'
slug: flux-review
type: post
status: publish
...
[Try Flux on fal.ai →](https://fal.ai/models/fal-ai/flux-pro) [AFFILIATE: flux]
...
| Variant | Access | Cost |
|--------|--------|------|
| FLUX.1 Schnell (open) | Local execution; fal.ai; Replicate | Free local; ~$0.003 per image cloud |
| FLUX.1 Dev (open, non-commercial) | Local execution; fal.ai; Replicate | Free local; cloud usage-based |
| FLUX.1.1 Pro | fal.ai; Replicate; BFL API | $0.04-0.055 per image |
| Via Leonardo.ai | Standard Leonardo.ai subscription | From $12/mo |
| Via Freepik | Standard Freepik AI subscription | From ~€11/mo |
...
| Data Point | Value |
|-----------|-------|
| Underlying model family | FLUX.1 (Schnell, Dev, Pro; FLUX.1.1 Pro) |
| Max output resolution | 2048×2048 native (Pro); upscaling on platforms |
| Style presets | Variable by access platform |
| Image-to-image | Yes |
| Inpainting / outpainting | Both (via supporting platforms / ControlNet-equivalent) |
| Negative prompting | Yes |
| Batch generation | Yes (platform-dependent) |
| Custom model / LoRA training | Yes (Schnell / Dev — open weights) |
| Generation speed | Fast (Schnell); Medium (Dev); Medium (Pro) |
| Commercial licensing included | Yes (Schnell + Pro); No (Dev — research only) |
...
- **[Midjourney](/midjourney-review/)** — higher default aesthetic polish; weaker prompt adherence.
- **[Stable Diffusion](/stable-diffusion-review/)** — more flexibility and larger ecosystem; weaker prompt following.
- **[Ideogram](/ideogram-review/)** — similar text-rendering capability; simpler first-party UX.
- **[Leonardo.ai](/leonardo-review/)** — easiest way to access Flux with a polished app.
- **[DALL-E 3](/dalle-3-review/)** — integrated into ChatGPT; easier but less flexible.
...
On prompt adherence and text rendering, yes. On default aesthetic polish and consumer UX, Midjourney is still easier to use and more opinionated. Many power users use both.

**Is Flux free?**
The Schnell variant is open weights under Apache 2.0 license — free to run locally and commercial-use-friendly. Dev is open but non-commercial. Pro is API-only with usage-based pricing.

**How do I access Flux?**
Through third-party platforms: Leonardo.ai and Freepik for consumer apps; fal.ai, Replicate, and BFL's own API for developers; ComfyUI and Draw Things for local execution.

**Is Flux commercially safe?**
Schnell and Pro outputs are commercial-use licensed. Dev is non-commercial license only. BFL does not currently offer Adobe-style indemnification.

**What hardware do I need for local Flux?**
Schnell runs on ~12GB VRAM at reasonable speed; Dev similar. Quality improves on 24GB+ cards. CPU-only execution is possible but very slow.
...
| Field | Value |
|-------|-------|
| Tool Name | Flux (Black Forest Labs) |
| Category | AI Image Generators |
...
| Price From | Free (Schnell local); ~$0.003 per image (cloud Schnell); $0.04-0.055 (Pro) |
| Free Plan | Yes (Schnell local / Apache 2.0) |
| Free Plan Limitations | Requires hardware; learning curve for direct API |
| Best For | Prompt-adherent generation and open-weights commercial use |
...
|------------|-------|
| Underlying model family | FLUX.1 (Schnell / Dev / Pro); FLUX.1.1 Pro |
| Max output resolution | 2048×2048 native (Pro); platform-dependent upscaling |
| Style presets | Variable by access platform |
| Image-to-image | Yes |
| Inpainting / outpainting | Both (via supporting platforms) |
| Negative prompting | Yes |
| Batch generation | Yes |
| Custom model / LoRA training | Yes (Schnell / Dev open weights) |
| Generation speed | Fast (Schnell); Medium (Dev / Pro) |
| Commercial licensing included | Yes (Schnell + Pro); No (Dev — research only) |
| Export formats | PNG, JPG, WebP |
